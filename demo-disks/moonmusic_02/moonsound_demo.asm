...
MoonSound_load:
		...
		call	MoonSound_init_opl4
		call	MoonSound_mwkload
...
MoonSound_tabl_music:
		...
		db	17h   				; 13 музыкальное произведение
		dw	0C000h
		dw	1E00h
...
MoonSound_mwkload:
		...
		ld	b,7				;число блоков по 16кб
MoonSound_loop_block:

		push	bc
		ld	hl,0C000h			;грузим блок семплов
		ld	de,(5CF4h)
		ld	bc,4005h			
		call	3d13h
		...
		ld	hl,0C000h
		ld	bc,4000h
		ld	d,06h                        	;06 -запишем данные данные

MoonSound_loop_write:
		ld	e,(hl)
		call	MoonSound_wave_out		
		call	MoonSound_busy			;проверим готовность микросхемы
		inc	hl
		dec	bc
		ld	a,b
		or	c
		jr	nz, MoonSound_loop_write
		pop	bc
		djnz	MoonSound_loop_block
...
Interrupt_handle:
		...
		in	a, (0C4h)				;проверим статусный регистр YM278
		rla	
		jr	nc, Interrupt_exit                      ;если прерывание не от YM278 - выход

		ld	a,4
		out	(0C4h),a
		nop	
		ld	a,128+1                         	
		out	(0C5h),a			        ;сбросим флаг прерывания
